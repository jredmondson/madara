  ...
  Knowledge::Update & data)
{
  if (data.key.val ())
  {
    ...
    MADARA_DEBUG (MADARA_LOG_MAJOR_DEBUG_INFO, (LM_DEBUG, 
        DLINFO "SpliceDataReaderListener::handle_assignment:" \
  ...
  Knowledge::Update & data)
{
  if (data.key.val ())
  {
    ...
    MADARA_DEBUG (MADARA_LOG_MAJOR_DEBUG_INFO, (LM_DEBUG, 
        DLINFO "SpliceDataReaderListener::multiassignment:" \
    ...
    while (!stream.eof ())
    {
      ...
      {
        MADARA_DEBUG (MADARA_LOG_MAJOR_EVENT, (LM_DEBUG, 
          DLINFO "SpliceDataReaderListener::handle_multiassignment:" \
          " received data[%s]=%q from %s.\n",
          key.c_str (), value, data.originator.val ()));
      }
      // if the data was already current
