...
int madara::transport::NddsTransport::setup(void)
{
  ...
  domain_participant_ =
      DDSDomainParticipantFactory::get_instance()->create_participant(domain,
  ...
  update_topic_ = domain_participant_->create_topic(topic_names_[0],
  ...
  listener_ = new NddsListener(settings_, id_, context_, send_monitor_,
    ...
    const knowledge::KnowledgeMap& updates)
{
  ...
  utility::strncpy_safe(data.originator, id_.c_str(), id_.size() + 1);
  ...
  utility::strncpy_safe(data.madara_id, MADARA_IDENTIFIER, strlen(MADARA_IDENTIFIER) + 1);
