...
{
  typedef std::map <unsigned int, unsigned int>  CandidateMap;

  Workflow & deployment = settings.target_deployment;
  SolutionMap & lookup = settings.solution_lookup;
...
void
madara::cid::approximate (SummationsMap & network_summations,
      Workflow & target_deployment, Deployment & solution, 
      SolutionMap & solution_lookup, unsigned int node)
{
  /**
  ...
  DirectedEdges & current_flow = target_deployment[node];
  ...
  if (degree > 0)
  {
    ...
    for (LatencyVector::size_type i = 0; i < cur_summations.size (); ++i)
    {
      /**
...
uint64_t
madara::cid::calculate_latency (LVVector & latencies, Workflow & workflow,
                                Deployment & solution)
{
  ...
  {
    DirectedEdges & current = workflow[i];
    ...
    if (current.size () > 0)
    {
      ...
      LatencyVector source_latencies = latencies[solution[source]];

      // sort by ID so we can use the array as a O(1) lookup
      std::sort (source_latencies.begin (), source_latencies.end (),
                 IncreasingId);

      for (DirectedEdges::size_type j = 0; j < current.size (); ++j)
      {
#ifdef ENABLE_CID_LOGGING
        MADARA_DEBUG (MADARA_LOG_DETAILED_TRACE, (LM_DEBUG, 
...
madara::cid::fill_by_highest_degree (Settings & settings, bool use_workflow)
{
  ...
  if (use_workflow)
  {
    ...
    for (unsigned int i = 0; i < deployment.size (); ++i)
    {
      ...
      if (source_flow.size () > 0)
      { 
        for (DirectedEdges::size_type j = 0; j < source_flow.size (); ++j)
        {
          unsigned int & dest = source_flow[j].second;
          unsigned int & dest_id = solution[dest];

          SolutionMap::iterator found = lookup.find (dest_id);
  ...
  {
    SolutionMap::iterator found = lookup.find (solution[i]);
...
madara::cid::fill_from_solution_map (Settings & settings)
{
  ...
  for (Workflow::size_type i = 0; i < deployment.size (); ++i)
  {
    ...
    if (source_flow.size () > 0)
    {
      unsigned int & source = source_flow[0].first;
      unsigned int & source_id = solution[source];
      LatencyVector & latencies = settings.network_latencies[source_id];
...
madara::cid::pathwise_approximate (Settings & settings,
             Deployment & solution, SolutionMap & lookup)
{
  ...
  for (Paths::size_type i = 0; i < paths.size (); ++i)
  {
    ...
    if (found == lookup.end () || found->second != source)
    {
      ...
      for (Paths::size_type j = 0; j < MAX_SOLUTIONS && j < paths.size (); ++j)
      {
        ...
        LinkVector & list = paths[i].list;
        LatencyVector & source_latencies = latencies[cur_summations[j].first];

        std::sort (source_latencies.begin (), source_latencies.end (),
          IncreasingId);

        for (LinkVector::size_type k = 0; k < list.size (); ++k)
        {
      ...
      //std::sort (utilities.begin (), utilities.end (), IncreasingLatency);
                             ...
                             unsigned int depth)
{
  ...
  for (DirectedEdges::iterator j = deployment[connected.source].begin ();
    j != deployment[connected.source].end (); ++j)
  {
    ...
    LinkMap::iterator found = source.dest.find (dest_id);
...
madara::cid::prepare_deployment (Settings & settings)
{
    ...
    i < deployment.size (); ++i)
  {
    if (deployment[i].size () != 0)
    {
      ...
      for (DirectedEdges::size_type j = 0; j < deployment[i].size (); ++j)
      {
         ...
         source < paths.size () && paths[source].dest.size () > 0; ++source)
  {
    ...
    LinkMap & dest_map = paths[source].dest;

    for (LinkMap::iterator dest = dest_map.begin ();
         ...
         i < paths.size () && paths[i].dest.size () > 0; ++i)
  {
    ...
    unsigned int cur = 0;
    // 
    for (LinkMap::iterator j = paths[i].dest.begin ();
  ...
  std::sort (deployment.begin (), deployment.end (),
             DecreasingSizeDirectedEdges);
  std::sort (paths.begin (), paths.end (), DecreasingSizeAndDegreesLinks);
       ...
       i != degrees.end (); ++i, ++k)
  {
    ...
    LatencyVector & summations = settings.network_summations[i->first];
