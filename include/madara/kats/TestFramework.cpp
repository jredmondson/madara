 ...
 *
 * This file contains the implementations for the TestFramework class
 */

/// Constructor
madara::kats::TestFramework::TestFramework (
  const madara::kats::Settings & settings)
: knowledge_ (settings.host, settings)
{
  knowledge_.set (".madara.id",
    madara::KnowledgeRecord::Integer (settings.id));
  knowledge_.set (".madara.processes",
    madara::KnowledgeRecord::Integer (settings.processes));
}

/// Copy constructor
madara::kats::TestFramework::TestFramework (
  const TestFramework & original)
...
madara::kats::TestFramework::get (const ::std::string & key)
{
...
int
madara::kats::TestFramework::set (const ::std::string & key,
...
long long
madara::kats::TestFramework::barrier (const std::string & event_name)
{
  ...
  madara::KnowledgeRecord::Integer id =
    knowledge_.get (".madara.id").to_integer ();
  madara::KnowledgeRecord::Integer processes =
    knowledge_.get (".madara.processes").to_integer ();

  if (processes <= 1)
  {
    MADARA_DEBUG (MADARA_LOG_MAJOR_EVENT, (LM_DEBUG, 
      DLINFO "TestFramework::barrier: .madara.processes <= 1."
      " Returning success.\n"));
    return 1;
  }
  ...
  for (madara::KnowledgeRecord::Integer i = 0; i < processes; ++i)
  {
    buffer << " && MADARA.BARRIER.";
    buffer << event_name;
    buffer << ".";
  ...
  log_message << "\nKATS::TestFramework::barrier: started: ";
...
void
madara::kats::TestFramework::sync (void)
{
  std::stringstream log_message;

  log_message.str ("");
  log_message << "KATS::TestFramework::sync: Starting ";
  log_message << "\n";
  ...
  log_message.str ("");
  log_message << "KATS::TestFramework::sync: Finished ";
  log_message << "\n";
...
void
madara::kats::TestFramework::barriered_sync (const std::string & event_name)
{
  std::stringstream buffer;
  std::stringstream log_message;

  log_message.str ("");
  log_message << "KATS::TestFramework::barriered_sync: Starting ";
  ...
  log_message.str ("");
  log_message << "KATS::TestFramework::barriered_sync: Finished ";
  log_message << event_name;
  log_message << "\n";
...
int 
madara::kats::TestFramework::event (const std::string & name, 
        ...
        bool barrier_this_event, bool close_transport)
{
  ...
  // default result is 
  int result = 0;
  madara::KnowledgeRecord::Integer eval_result = 0;
...
void 
madara::kats::TestFramework::log (const std::string & statement, unsigned int level)
{
  knowledge_.print (statement, level);
}

/// Dumps all knowledge to std::err
void madara::kats::TestFramework::dump (unsigned int level)
{
