...
public:
  ...
  {
    int result = map_.set(std::forward<K>(key), std::forward<V>(val), settings);
  ...
  {
    int result =
        map_.set_index(std::forward<K>(key), i, std::forward<V>(val), settings);
  ...
  int set_text(const VariableReference& variable, const char* value,
      size_t size, const EvalSettings& settings = EvalSettings());
   ...
   **/
  int64_t load_context(const std::string& filename, bool use_id,
      const KnowledgeUpdateSettings& settings = KnowledgeUpdateSettings(
   ...
   **/
  MADARA_EXPORT int send_modifieds(const std::string& prefix,
...
private:
  struct ModifiedsSender
  {
    ...
    ~ModifiedsSender()
    {
      self->send_modifieds(func, *settings);
    }
  };
