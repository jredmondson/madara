...
bool operator< (
  const madara::knowledge::Timed_Event & lhs,
  const madara::knowledge::Timed_Event & rhs)
{
  return lhs.first > rhs.first;
}


madara::knowledge::Timed_Executor::Timed_Executor ()
...
void
madara::knowledge::Timed_Executor::add (const Timed_Event & new_event)
{
  Guard guard (mutex_);
...
void
madara::knowledge::Timed_Executor::add (const Event & new_event)
{
  Guard guard (mutex_);
...
ACE_Time_Value
madara::knowledge::Timed_Executor::remove (Timed_Event & cur_event)
{
  // obtain current time
  ACE_Time_Value cur_time = ACE_High_Res_Timer::gettimeofday ();
  ...
  if (events_.size () > 0)
  {
    madara_logger_ptr_log (logger::global_logger.get(), logger::LOG_MINOR, "Timed_Executor::remove: " \
    ...
    {
      madara_logger_ptr_log (logger::global_logger.get(), logger::LOG_MAJOR, "Timed_Executor::remove: "
...
void
madara::knowledge::Timed_Executor::shutdown (void)
{
  if (thread_info_)
  {
...
bool
madara::knowledge::Timed_Executor::is_shutdown (void)
{
  return control_plane_.get (terminated_).to_integer () != 0;
}

void
madara::knowledge::Timed_Executor::launch_threads (
...
ACE_Time_Value
madara::knowledge::Timed_Executor::time_until_next (void)
{
...
void 
madara::knowledge::Timed_Executor::lock (void)
{
  mutex_.acquire ();
}

void 
madara::knowledge::Timed_Executor::unlock (void)
{
  mutex_.release ();
}
...
void 
madara::knowledge::Timed_Executor::enter_barrier (void)
{
  Wait_Settings settings;
  settings.poll_frequency = .5;
  settings.max_wait_time = -1;

  madara_logger_ptr_log (logger::global_logger.get(), logger::LOG_MAJOR, "Timed_Executor::enter_barrier: "
    "Entering barrier\n");
...
void 
madara::knowledge::Timed_Executor::clear_queue (void)
{
  Guard guard (mutex_);
...
madara::knowledge::Timed_Executor::num_threads (void)
{
  Guard guard (mutex_);
