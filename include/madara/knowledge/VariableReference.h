  ...
  namespace knowledge
  {
    ...
    public:
      ...
      VariableReferenceMapReader(
          const VariableReferenceMap &map,
          bool history = false,
          uint64_t min_toi = 0)
      ...
      private:
        ...
        using biter_type =
          utility::CircularBuffer<KnowledgeRecord>::const_iterator;
        ...
        const_iterator(const VariableReferenceMapReader &f)
          : reader_(&f),
            miter_(reader_->map_.begin()),
            biter_(begin_biter()) {}

        const_iterator(
            const VariableReferenceMapReader &f,
            miter_type miter)
          : reader_(&f), miter_(miter), biter_(begin_biter()) {}

      public:
        const_iterator(const const_iterator &o)
          : reader_(o.reader_), miter_(o.miter_),
