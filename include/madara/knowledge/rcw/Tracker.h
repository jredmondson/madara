...
class Tracker<T, R, false, false, void> : public BaseTracker
{
private:
  static_assert(
      sizeof(T) < 0, "Cannot create tracker that can neither read nor write");
};

/// Tracker specialization for types that don't track modification status,
/// and that can both read and write
template<class T>
class Tracker<T, VariableReference, true, true,
    typename std::enable_if<
        supports_get_value<T>::value && supports_knowledge_cast<T>::value &&
        supports_self_eq<T>::value && !supports_is_dirty<T>::value>::type>
  : public BaseTracker
{
private:
...
template<class T>
class Tracker<T, VariableReference, true, false,
...
template<class T>
class Tracker<T, VariableReference, false, true,
    typename std::enable_if<
        supports_get_value<T>::value && supports_knowledge_cast<T>::value &&
        supports_self_eq<T>::value && !supports_is_dirty<T>::value>::type>
  : public BaseTracker
...
class Tracker<T, VariableReference, RD, WR,
    typename std::enable_if<
        supports_indexed_get_value<T>::value && supports_size<T>::value &&
        !supports_is_all_dirty<T>::value && !supports_is_size_dirty<T>::value &&
        supports_indexed_is_dirty<T>::value>::type> : public BaseTracker
{
private:
  static_assert(
      RD == true, "Write-only not supported for containers of tracked items");
...
class Tracker<T, VariableReference, RD, WR,
    typename std::enable_if<
        ...
        supports_size<T>::value && supports_knowledge_cast<T>::value &&
        supports_is_dirty<T>::value && supports_is_all_dirty<T>::value &&
