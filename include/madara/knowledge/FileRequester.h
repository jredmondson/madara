...
public:
   ...
   **/
  FileRequester(const std::string& prefix, const std::string& sync_key,
      const std::string& filename, KnowledgeBase kb,
   ...
   *                   in a single send_modifieds
   **/
  inline void init(const std::string& prefix, const std::string& sync_key,
      const std::string& filename, KnowledgeBase kb,
      int max_request_fragments = -1)
  {
    // set the sync key
