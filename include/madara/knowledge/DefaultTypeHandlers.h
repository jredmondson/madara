...
constexpr bool madara_ignore_for_each_field(...)
{
  return false;
}
...
inline std::vector<knowledge::AnyField> get_fields(T& val)
{
  ...
  for_each_field(
      do_list_fields{&knowledge::get_type_handler<T>(), &ret, 0}, val);
    ...
    type<T>, overload_priority<8>)
{
  return
      [](const knowledge::AnyField& field,
          const knowledge::TypeHandlers*& handler, void*& out_ptr, void* ptr) {
        T& val = *static_cast<T*>(ptr);
...
inline std::vector<knowledge::AnyField> get_members(type<T>)
{
  ...
  for_each_member(type<T>{},
      do_list_members<T>{&knowledge::get_type_handler<T>(), &ret, 0});
  std::sort(ret.begin(), ret.end(), knowledge::compare_any_fields_by_name());
  return ret;
}
    ...
    type<T>, overload_priority<8>)
{
  return
      [](const knowledge::AnyField& field,
        ...
        T& val = *static_cast<T*>(ptr);
        for_each_member(
            type<T>{}, do_get_member<T>{&val, &field, &handler, &out_ptr, 0});
      };
}
...
constexpr knowledge::TypeHandlers::index_int_fn_type get_type_handler_index_int(
    type<T>, overload_priority<8>)
{
  return [](size_t index, const knowledge::TypeHandlers*& handler,
...
template<typename T, enable_if_<knowledge::supports_int_index<T>::value &&
                                    knowledge::supports_size_member<T>::value &&
                                    knowledge::supports_resize_member<T>::value,
                         int> = 0>
constexpr knowledge::TypeHandlers::index_int_fn_type get_type_handler_index_int(
    type<T>, overload_priority<4>)
{
  return [](size_t index, const knowledge::TypeHandlers*& handler,
             void*& out_ptr, void* ptr) {
    T& val = *static_cast<T*>(ptr);
...
constexpr knowledge::TypeHandlers::index_str_fn_type get_type_handler_index_str(
    type<T>, overload_priority<8>)
{
  return [](const char* index, const knowledge::TypeHandlers*& handler,
             void*& out_ptr, void* ptr) {
    T& val = *static_cast<T*>(ptr);
    using I = decltype(val[index]);
    handler = &knowledge::get_type_handler<decay_<I>>();
...
constexpr knowledge::TypeHandlers::size_fn_type get_type_handler_size(
    type<T>, overload_priority<8>)
{
