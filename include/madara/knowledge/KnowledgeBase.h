...
public:
   ...
   **/
  KnowledgeBase(
      const std::string& host, int transport, const std::string& domain);
   ...
   **/
  void copy(const KnowledgeBase& source, const KnowledgeRequirements& reqs,
   ...
   * @param  settings  settings for appplying updates to context
   **/
  void copy(const KnowledgeBase& source, const CopySet& copy_set = CopySet(),
      bool clean_copy = false, const EvalSettings& settings = EvalSettings());
   ...
   */
  int read_file(const std::string& knowledge_key, const std::string& filename,
      const EvalSettings& settings = EvalSettings(
   ...
   * @return   0 if the value was set. -1 if null key
   **/
  int set(const VariableReference& variable, std::string&& value,
      const EvalSettings& settings = EvalSettings(
          true, false, true, false, false));
   ...
   * @return   0 if the value was set. -1 if null key
   **/
  int set(const VariableReference& variable, const KnowledgeRecord& value,
      const EvalSettings& settings = EvalSettings(
          true, false, true, false, false));
   ...
   * @return   0 if the value was set. -1 if null key
   **/
  int set(const VariableReference& variable, KnowledgeRecord&& value,
      const EvalSettings& settings = EvalSettings(
          true, false, true, false, false));
   ...
   * @return   0 if the value was set. -1 if null key
   **/
  int set(const std::string& key, const KnowledgeRecord& value,
      const EvalSettings& settings = EvalSettings(
          true, false, true, false, false));
   ...
   * @return   0 if the value was set. -1 if null key
   **/
  int set(const std::string& key, KnowledgeRecord&& value,
      const EvalSettings& settings = EvalSettings(
          true, false, true, false, false));
   ...
   * @return   0 if the value was set. -1 if null key
   **/
  int set(const VariableReference& variable, const std::vector<double>& value,
      const EvalSettings& settings = EvalSettings(
          true, false, true, false, false));
   ...
   * @return   0 if the value was set. -1 if null key
   **/
  int set(const VariableReference& variable, std::vector<double>&& value,
      const EvalSettings& settings = EvalSettings(
          true, false, true, false, false));
   ...
   * @return   0 if the value was set. -1 if null key
   **/
  int set(const VariableReference& variable, const double* value, uint32_t size,
      const EvalSettings& settings = EvalSettings(
          true, false, true, false, false));
  ...
  madara::knowledge::KnowledgeRecord retrieve_index(
      ...
      const KnowledgeReferenceSettings& settings = EvalSettings(
          true, false, true, false, false));
   ...
   * @throw exceptions::KarlException  failure during compile/evaluate
   **/
  madara::knowledge::KnowledgeRecord evaluate(CompiledExpression& expression,
      const EvalSettings& settings = EvalSettings());
   ...
   * @throw exceptions::KarlException  failure during compile/evaluate
   **/
  madara::knowledge::KnowledgeRecord evaluate(expression::ComponentNode* root,
      const EvalSettings& settings = EvalSettings());
  ...
  void define_function(
      const std::string& name, knowledge::KnowledgeRecord (*func)(const char*,
   ...
   *                        -2 if quality isn't high enough
   **/
  int set(const std::string& key, std::vector<KnowledgeRecord::Integer>&& value,
      const EvalSettings& settings = EvalSettings(
          true, false, true, false, false));
   ...
   * @return   0 if the value was set. -1 if null key
   **/
  int set(const std::string& key, const EvalSettings& settings = EvalSettings(
                                      true, false, true, false, false))
  {
   ...
   *                        -2 if quality isn't high enough
   **/
  int set(const std::string& key, std::vector<double>&& value,
      const EvalSettings& settings = EvalSettings(
          true, false, true, false, false));
  ...
  int64_t save_checkpoint(
      const std::string& filename, bool reset_modifieds = true);
   ...
   * @throw exceptions::MemoryException  not enough buffer to encode
   **/
  int64_t load_context(const std::string& filename, bool use_id = true,
      const KnowledgeUpdateSettings& settings = KnowledgeUpdateSettings(
...
public:
  ...
  size_t get_oldest(const std::string& key, OutputIterator out, size_t count,
  ...
  auto get_oldest(const std::string& key, OutputIterator out,
   ...
   **/
  template<typename OutputIterator>
  size_t get_newest(const std::string& key, OutputIterator out, size_t count,
      const KnowledgeReferenceSettings& settings =
          KnowledgeReferenceSettings()) const
  {
    return invoke(key,
   ...
   **/
  template<typename OutputIterator, typename ConstOutputIterator>
  auto get_newest(const std::string& key, OutputIterator out,
      ConstOutputIterator out_end,
      const KnowledgeReferenceSettings& settings =
          KnowledgeReferenceSettings()) const
      -> enable_if_<!std::is_arithmetic<ConstOutputIterator>::value, size_t>
   ...
   * a vector.
   **/
  std::vector<KnowledgeRecord> get_oldest(const std::string& key, size_t count,
      const KnowledgeReferenceSettings& settings =
          KnowledgeReferenceSettings()) const
  {
    return invoke(key,
   ...
   * a vector.
   **/
  std::vector<KnowledgeRecord> get_newest(const std::string& key, size_t count,
      const KnowledgeReferenceSettings& settings =
          KnowledgeReferenceSettings()) const
  {
    return invoke(key,
   ...
   **/
  template<typename OutputIterator>
  size_t get_history(const std::string& key, OutputIterator out, ssize_t index,
      size_t count,
      const KnowledgeReferenceSettings& settings =
          KnowledgeReferenceSettings()) const
  {
   ...
   **/
  template<typename OutputIterator>
  auto get_history(const std::string& key, OutputIterator out,
      const KnowledgeReferenceSettings& settings =
          KnowledgeReferenceSettings()) const -> decltype(*out, size_t{})
  {
    return invoke(key,
   ...
   * indices count from newest entries (-1 is newest).
   **/
  std::vector<KnowledgeRecord> get_history(const std::string& key, size_t index,
      size_t count,
      const KnowledgeReferenceSettings& settings =
          KnowledgeReferenceSettings()) const
  {
   ...
   * indices count from newest entries (-1 is newest).
   **/
  KnowledgeRecord get_history(const std::string& key, size_t index,
      const KnowledgeReferenceSettings& settings =
          KnowledgeReferenceSettings()) const
  {
    return invoke(key,
