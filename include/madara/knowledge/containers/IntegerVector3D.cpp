...
std::string madara::knowledge::containers::IntegerVector3D::get_debug_info_(
    void)
{
...
void madara::knowledge::containers::IntegerVector3D::modify(
    const Indices& index)
{
  if (context_) {
    ContextGuard context_guard(*context_);
...
void madara::knowledge::containers::IntegerVector3D::operator=(
    const IntegerVector3D& rhs)
{
  if (this != &rhs) {
...
void madara::knowledge::containers::IntegerVector3D::resize(
    const Dimensions& dimensions, bool delete_vars)
{
  if (context_ && name_ != "") {
    ContextGuard context_guard(*context_);
...
void madara::knowledge::containers::IntegerVector3D::set_name(
...
void madara::knowledge::containers::IntegerVector3D::set_delimiter(
    const std::string& delimiter)
{
  delimiter_ = delimiter;
  if (context_) {
...
void madara::knowledge::containers::IntegerVector3D::copy_to(
  ...
  if (context_) {
    ...
    for (size_t i = 0; i < dimensions.x; ++i) {
      ...
      for (size_t j = 0; j < dimensions.y; ++j) {
        ...
        for (size_t k = 0; k < dimensions.z; ++k) {
          target[i][j][k] =
              context_->get(vector_[i][j][k], keep_local).to_integer();
        }
      }
    }
  }
...
bool madara::knowledge::containers::IntegerVector3D::exists(
    const Indices& index) const
{
  bool result(false);
...
int madara::knowledge::containers::IntegerVector3D::set(
...
void madara::knowledge::containers::IntegerVector3D::set_quality(
...
bool madara::knowledge::containers::IntegerVector3D::is_true(void) const
{
  ...
  if (context_) {
    ...
    madara_logger_log(context_->get_logger(), logger::LOG_MINOR,
        "IntegerVector3D::is_true: context was not null. Result changed to "
        "%d\n",
        (int)result);
