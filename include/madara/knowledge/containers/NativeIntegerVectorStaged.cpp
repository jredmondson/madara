...
: BaseContainer (rhs), context_ (rhs.context_),
  vector_ (rhs.vector_), has_changed_ (false)
{
  madara_logger_ptr_log (logger::global_logger.get (),
    logger::LOG_MAJOR,
...
bool
madara::knowledge::containers::NativeIntegerVectorStaged::is_false_ (void) const
{
  return is_false ();
}

bool
madara::knowledge::containers::NativeIntegerVectorStaged::is_true_ (void) const
{
  return is_true ();
}

inline void
madara::knowledge::containers::NativeIntegerVectorStaged::modify_ (void)
{
  modify ();
}
