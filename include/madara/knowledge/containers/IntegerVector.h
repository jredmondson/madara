...
public:
   ...
   * @param  delimiter  the delimiter for variables in the vector
   **/
  IntegerVector(const std::string& name, KnowledgeBase& knowledge,
   ...
   * @param  delimiter  the delimiter for variables in the vector
   **/
  IntegerVector(const std::string& name, Variables& knowledge, int size = -1,
   ...
   **/
  void exchange(
      IntegerVector& other, bool refresh_keys = true, bool delete_keys = true);
  ...
  int set(size_t index,
      const type& value = madara::knowledge::KnowledgeRecord::MODIFIED);
