...
madara::knowledge::containers::IntegerVector2D::IntegerVector2D (
...
void
madara::knowledge::containers::IntegerVector2D::modify (void)
{
  if (context_ && name_ != "")
  {
...
std::string
madara::knowledge::containers::IntegerVector2D::get_debug_info (void)
{
  std::stringstream result;

  result << "IntegerVector2D: ";
...
void
madara::knowledge::containers::IntegerVector2D::modify_ (void)
{
  modify ();
}

std::string
madara::knowledge::containers::IntegerVector2D::get_debug_info_ (void)
{
  return get_debug_info ();
}

madara::knowledge::containers::BaseContainer *
madara::knowledge::containers::IntegerVector2D::clone (void) const
{
  return new IntegerVector2D (*this);
}

void
madara::knowledge::containers::IntegerVector2D::modify (const Indices & index)
{
  if (context_)
  {
    ContextGuard context_guard (*context_);
...
void
madara::knowledge::containers::IntegerVector2D::operator= (
  const IntegerVector2D & rhs)
{
  if (this != &rhs)
  {
...
madara::knowledge::containers::IntegerVector2D::get_size_ref (void)
{
...
void
madara::knowledge::containers::IntegerVector2D::resize (
  const Dimensions & dimensions, bool delete_vars)
{
  if (context_ && name_ != "")
  {
    ...
    madara_logger_log (context_->get_logger (), logger::LOG_MAJOR,
      "IntegerVector2D::resize: resizing to [%d,%d]\n",
    ...
    madara_logger_log (context_->get_logger (), logger::LOG_MINOR,
      "IntegerVector2D::resize: old size is [%d,%d]\n",
    (int)old_size.x, (int)old_size.y);

    if (is_reset)
    {
      madara_logger_log (context_->get_logger (), logger::LOG_MINOR,
        "IntegerVector2D::resize: new size is being reset to size in KB\n");
    ...
    else
    {
      ...
      madara_logger_log (context_->get_logger (), logger::LOG_MAJOR,
        "IntegerVector2D::resize: using dimensions passed in.\n");
    ...
    for (size_t i = 0; i < new_size.x; ++i)
    {
      madara_logger_log (context_->get_logger (), logger::LOG_DETAILED,
        "IntegerVector2D::resize: resizing vector_[%d] to %d.\n",
      ...
      madara_logger_log (context_->get_logger (), logger::LOG_MINOR,
        "IntegerVector2D::resize: creating var_refs from %d->%d.\n",
    ...
    if (new_size.x < old_size.x || new_size.y < old_size.y && delete_vars)
    {
      ...
      for (size_t i = 0; i < old_size.x; ++i)
      {
        ...
        madara_logger_log (context_->get_logger (), logger::LOG_MINOR,
          "IntegerVector2D::resize: deleting refs: %d:%d->%d.\n",
        ...
        for (size_t j = start; j < old_size.y; ++j)
        {
          ...
          madara_logger_log (context_->get_logger (), logger::LOG_DETAILED,
            "IntegerVector2D::resize: deleting ref: %s.\n",
            var_name.str ().c_str ());
...
madara::knowledge::containers::IntegerVector2D::Indices
madara::knowledge::containers::IntegerVector2D::size (void) const
{
  Indices cur_size;
...
void
madara::knowledge::containers::IntegerVector2D::set_name (
...
void
madara::knowledge::containers::IntegerVector2D::set_delimiter (
const std::string & delimiter)
{
  delimiter_ = delimiter;
  if (context_)
...
std::string
madara::knowledge::containers::IntegerVector2D::get_delimiter (void)
{
  return delimiter_;
}

void
madara::knowledge::containers::IntegerVector2D::copy_to (
  std::vector <std::vector <type> > & target) const
{
  ...
  if (context_)
  {
    ...
    for (size_t i = 0; i < dimensions.x; ++i)
    {
      ...
      for (size_t j = 0; j < dimensions.y; ++j)
      {
        target[i][j] = context_->get (vector_[i][j], keep_local).to_integer ();
      }
    }
  }
}

madara::knowledge::containers::IntegerVector2D::type
madara::knowledge::containers::IntegerVector2D::operator[] (
  const Indices & index) const
{
  ...
  if (context_)
  {
    ...
    {
      result = context_->get (
        vector_[index.x][index.y], keep_local).to_integer ();
    }
  }
...
bool
madara::knowledge::containers::IntegerVector2D::exists (
  const Indices & index) const
{
  bool result (false);
...
int
madara::knowledge::containers::IntegerVector2D::set (
const Indices & index,
type value)
{
  int result = -1;
...
void
madara::knowledge::containers::IntegerVector2D::set_quality (
  const Indices & index,
  uint32_t quality,
  const KnowledgeReferenceSettings & settings)
{
  if (context_)
  {
    ...
    if (index.x < vector_.size () && index.y < vector_[index.x].size ())
      context_->set_quality (vector_[index.x][index.y].get_name (), quality,
        true, settings);
  }
}

bool
madara::knowledge::containers::IntegerVector2D::is_true (void) const
{
  bool result (false);

  madara_logger_log (context_->get_logger (), logger::LOG_MAJOR,
    "IntegerVector2D::is_true: Checking for truth\n");
  ...
  if (context_)
  {
    ...
    madara_logger_log (context_->get_logger (), logger::LOG_MINOR,
      "IntegerVector2D::is_true: context was not null. Result changed to %d\n",
      (int)result);
    ...
    for (size_t i = 0; i < vector_.size (); ++i)
    {
      ...
      {
      madara_logger_log (context_->get_logger (), logger::LOG_DETAILED,
        "IntegerVector2D::is_true: checking [%d,%d], is_false of %d. \n",
        (int)i, (int)j, (int)context_->get (vector_[i][j]).is_false ());

        if (context_->get (vector_[i][j]).is_false ())
        {
          madara_logger_log (context_->get_logger (), logger::LOG_MAJOR,
            "IntegerVector2D::is_true: result is false, breaking\n");
  ...
  madara_logger_log (context_->get_logger (), logger::LOG_MAJOR,
    "IntegerVector2D::is_true: final result is %d\n", (int)result);

  return result;
}

bool
madara::knowledge::containers::IntegerVector2D::is_false (void) const
{
  return !is_true ();
}
...
bool
madara::knowledge::containers::IntegerVector2D::is_true_ (void) const
{
  return is_true ();
}

bool
madara::knowledge::containers::IntegerVector2D::is_false_ (void) const
{
  return is_false ();
}
