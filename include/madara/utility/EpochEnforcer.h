...
public:
  ...
  inline void set_duration(double duration)
  {
    ...
    auto duration_ns =
        std::chrono::duration_cast<std::chrono::nanoseconds>(temp_duration);
  ...
  inline void set_period(double period)
  {
    ...
    period_ = std::chrono::duration_cast<std::chrono::nanoseconds>(temp_period);
  ...
  {
    auto dur = std::chrono::duration_cast<PERIOD>(CLOCK::now() - start_);
