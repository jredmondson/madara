...
madara::expression::SystemCallPrintSystemCalls::prune(bool& can_change)
{
  ...
  if (calls_.size() == 0)
  {
    ...
    calls_["#eval"] =
        ...
        "  Evaluates the KaRL expression and returns a result. Works "
        "similarly\n"
    ...
    const madara::knowledge::KnowledgeUpdateSettings& settings)
{
  ...
  if (nodes_.size() == 1)
  {
    ...
    context_.print(calls_[nodes_[0]->evaluate(settings).to_string()], 0);
...
// accept a visitor
void madara::expression::SystemCallPrintSystemCalls::accept(
