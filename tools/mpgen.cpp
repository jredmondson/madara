  ...
  for(int i = 1; i < argc; ++i)
  {
    ...
    else
    {
      std::cerr
          ...
          << " [options]:\n\n"
             "Generates a platform-neutral project file for compiling MADARA "
             "apps."
             "\n\noptions:\n"
...
bool copy_mpbs(void)
{
  ...
  if(madara_root != "")
  {
    ...
    std::string using_ace =
        utility::file_to_string(madara_root + "/using_ace.mpb");
    std::string using_madara =
        utility::file_to_string(madara_root + "/using_madara.mpb");

    // write mpb files
    utility::write_file(output_dir + "/using_ace.mpb", (void*)using_ace.c_str(),
        using_ace.size());
