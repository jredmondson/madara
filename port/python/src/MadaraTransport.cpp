...
{
  object transport = object(handle<>(PyModule_New("madara.transport")));
  ...
  extract<dict>(getattr(import("sys"), "modules"))()["madara.transport"] =
      transport;
  ...
  enum_<madara::transport::Types>("TransportTypes")
      .value("NO_TRANSPORT", madara::transport::NO_TRANSPORT)
      .value("SPLICE", madara::transport::SPLICE)
      .value("NDDS", madara::transport::NDDS)
      .value("TCP", madara::transport::TCP)
      .value("UDP", madara::transport::UDP)
      .value("MULTICAST", madara::transport::MULTICAST)
      .value("BROADCAST", madara::transport::BROADCAST)
      .value("REGISTRY_SERVER", madara::transport::REGISTRY_SERVER)
      .value("REGISTRY_CLIENT", madara::transport::REGISTRY_CLIENT)
      .value("ZMQ", madara::transport::ZMQ);

  {
    ...
    scope TransportContext =
        class_<madara::transport::TransportContext>("TransportContext",
            ...
            .def("add_record", &madara::transport::TransportContext::add_record,
                "Adds a record to the context for rebroadcasting or applying")

            // Retrieves the knowledge domain (similar to topics in pub/sub)
            .def("get_domain", &madara::transport::TransportContext::get_domain,
  ...
  class_<madara::transport::TransportSettings>(
      "TransportSettings", "The main transport settings class", init<>())
      ...
      .def_readwrite("type", &madara::transport::TransportSettings::type,
      ...
      .def_readwrite("id", &madara::transport::TransportSettings::id,
      ...
      .def_readwrite("hosts", &madara::transport::TransportSettings::hosts,
  ...
  class_<madara::transport::QoSTransportSettings,
      bases<madara::transport::TransportSettings> >("QoSTransportSettings",
      ...
      .def("add_rebroadcast_filter",
          static_cast<void (madara::transport::QoSTransportSettings::*)(
