...
JNIEXPORT jstring JNICALL Java_ai_madara_knowledge_AnyRef_jni_1field(
    JNIEnv* env, jclass, jstring name, jlong handler, jlong data,
    jlongArray out)
{
  return catch_wrap(env, [&]() {
    AnyRef a{make_anyref(handler, data)};
...
JNIEXPORT jstring JNICALL Java_ai_madara_knowledge_AnyRef_jni_1assign__JJJJ(
    JNIEnv* env, jclass, jlong lhandler, jlong ldata, jlong rhandler,
    jlong rdata)
{
  return catch_wrap(env, [&]() {
...
union NoDestruct {
  ...
  NoDestruct(const char* tag) : a(Any::construct(tag)) {}
 ...
 */
MADARA_EXPORT jstring JNICALL
Java_ai_madara_knowledge_KnowledgeBase_jni_1setAnySettings(JNIEnv* env, jclass,
    jlong cptr, jstring name, jlong handler, jlong data, jlong settings_ptr)
{
  KnowledgeBase* knowledge = (KnowledgeBase*)cptr;
    ...
    JNIEnv* env, jclass, jlong handler, jlong data, jobjectArray out)
{
  return catch_wrap(env, [&]() {
    ...
    env->SetByteArrayRegion(ret, 0, buf.size(), (jbyte*)buf.begin());
    env->SetObjectArrayElement(out, 0, ret);
  });
}
