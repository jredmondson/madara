...
Broker_Timeout_Handler::Broker_Timeout_Handler (Madara::Broker_Context & context)
: context_ (context)
{
...
int 
Broker_Timeout_Handler::check_key (const std::string &key)
{
  std::string this_key;
...
int 
Broker_Timeout_Handler::random_discover (void)
{
...
int 
Broker_Timeout_Handler::random_latency_query (void)
{
  int num_keys = context_.getNumKeys ();
...
int
Broker_Timeout_Handler::dump_latencies (void)
{
...
int 
Broker_Timeout_Handler::handle_timeout (const ACE_Time_Value &tv, const void *arg)
{
