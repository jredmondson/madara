 ...
 */
static const char *const SERVER_PORT = "127.0.0.1";
...
/*    char buf[28];
    ...
    // first part tells the receiver how big the message is

    *i_value = 24;
    ++i_value;

    // next part tells the receiver what type of message it is

    *i_value = 1;
    ++i_value;

    // fill in the host ip
    c_value = (char *)i_value;

    for (j = 0; j < server_host.size (); ++j, ++c_value)
      *c_value = server_host[j];

    // clear out the rest of the buf reserved for ip (min 1 char)
    for (; j < 16; ++j, ++c_value)
      *c_value = 0;
    ...
    *i_value = atoi (port.c_str ());

    ACE_DEBUG ((LM_DEBUG, "(%P|%t) Message: \n", server_port));
