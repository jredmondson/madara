...
derive our ClientHandler directly from ACE_Event_Handler but you still have
...
class ClientHandler : public ACE_Svc_Handler < ACE_SOCK_STREAM, ACE_NULL_SYNCH >
  ...
  public:
    ...
    */
    ...
    creating a new ClientHandler when a client connects. Notice that the
  ...
  protected:
    ...
    /*
    If the ClientAcceptor which created us has chosen a thread-per-connection
    ...
    */
    ...
    int process_dump (Madara::AgentPing& from);
    int process_deployment_offer (Madara::AgentPing& from);
    int process_deployment_print (Madara::AgentPing& from);


    //int process_latency_query (Madara::AgentPing& from);
    /*
    ...
    */
    ...
    /*
    When we get to the definition of ClientHandler we'll see that there are
    several places where we go back to the ClientAcceptor for information.
    It is generally a good idea to do that through an accesor rather than
    using the member variable directly.
    */
    ClientAcceptor * client_acceptor( void )
