...
int ClientHandler::process_ping (Madara::AgentPing& from)
{
...
ACE_DEBUG ((LM_DEBUG, "(%P|%t) client %s:%d wants to play\n", 
  ...
  this->peer ().recv ( &from, sizeof (Madara::AgentPing), &timeout);
...
int ClientHandler::process_latency_query (Madara::AgentPing& from)
{
  Madara::AgentPing reply;
  std::string key;
  std::string host;
  ...
  Madara::AgentLatency * latencies = (Madara::AgentLatency *)
    malloc (sizeof (Madara::AgentLatency) * reply.size);
  ...
  this->peer ().send_n (&reply, sizeof(Madara::AgentPing), &timeout);
  ...
  this->peer ().send_n (latencies, 
    sizeof (Madara::AgentLatency) * reply.size, 
