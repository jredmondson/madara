...
void test_hive(void)
{
  ...
  std::cerr << "Hive: Test 1: from agent.0: agent.0.name == Sam Brandy: ";
  ...
  std::cerr << "Hive: Test 2: from agent.1: agent.1.name == Bob Franklin: ";
  ...
  std::cerr << "Hive: Test 3: from agent.2: agent.2.name == Sara Thompkins: ";
  ...
  if (madara_fails > 0)
  {
    std::cerr << "Hive: OVERALL: FAIL. " << madara_fails << " tests failed.\n";
  }
  else
  {
...
int main(int, char**)
{
  ...
  {
    transports[i] =
        new transport::SharedMemoryPush(kbs[i].get_id(), settings, kbs[i]);
  ...
  if (madara_fails > 0)
  {
    std::cerr << "SharedPush: OVERALL: FAIL. " << madara_fails <<
      " tests failed.\n";
  }
  else
  {
