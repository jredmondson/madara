  ...
  for(int i = 1; i < argc; ++i)
  {
    ...
    else
    {
      madara_logger_ptr_log(logger::global_logger.get(), logger::LOG_ALWAYS,
          ...
          "  of rebroadcasts (-e|--rebroadcasts controls the number of "
          "rebroadcasts)\n\n"
...
void write_file(madara::knowledge::KnowledgeMap& records,
    ...
    madara::knowledge::Variables& vars)
{
  ...
  if(last != records.end() && last->second == trusted.to_string())
  {
    ...
    if(file != records.end())
    {
      ...
      vars.set(ack,
          madara::knowledge::KnowledgeRecord::Integer(file->second.size()));
      vars.print(
...
int main(int argc, char** argv)
{
  ...
  knowledge.set(".target", target_id, madara::knowledge::EvalSettings::SEND);
