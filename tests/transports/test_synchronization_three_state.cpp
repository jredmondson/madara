...
"3 State Self-Stabilizing Distributed Test (from Dijkstra 1974):\n\n" 
  ...
  for (int i = 1; i < argc; ++i)
  {
    ...
    else if (arg1 == "-n" || arg1 == "--num-processes")
    {
      if (i + 1 < argc && argv[i + 1][0] != '-')
      {
        ...
        if (num_processes < 3)
        {
          std::cerr << "Warning: num_processes must be >= 3. Changing to 3.\n";
          num_processes = 3;
        }
      }
      else
    ...
    else if (arg1 == "-s" || arg1 == "--initial-state")
    {
      if (i + 1 < argc)
      {
        ...
        if (initial_state > 2 || initial_state < 0)
        {
          std::cerr << "State must be within 3 state system (0, 1, 2).";
          std::cerr << " Changing to 2.\n";
          initial_state = 2;
        }
      }
      else
...
int main(int argc, char ** argv)
{
  ...
  */
  ...
  while (!enforcer.is_done ())
  {
     ...
     * for the top machine:
     *   if L = R and (L+l)mod 3 != S then S := (L+1)mod 3 fi
    ...
    else if (id == num_processes - 1)
    {
      (L == R) && (*L + 1) % 3 != *S ? S = (*L + 1) % 3 : 0;
    }
    else
    {
      (*S + 1) % 3 == *L ? S = *L : 0;
