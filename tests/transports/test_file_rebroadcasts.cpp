  ...
  for(int i = 1; i < argc; ++i)
  {
    ...
    else
    {
      madara_logger_ptr_log(logger::global_logger.get(), logger::LOG_ALWAYS,
          ...
          "  of rebroadcasts (-h|--hops controls the number of "
          "rebroadcasts)\n\n"
          ...
          " [-s|--size size]         size of packet to send in bytes "
          "(overrides file)\n"
          " [-si|--size-increment inc] increment of size from start to end "
          "(see -se)\n"
          " [-se|--size-end end]     last automated size to test (see -s and "
          "-si)\n"
          " [-st|--slack-time time]  time in seconds to sleep between fragment "
          "sends\n"
          "                          (.001 seconds by default)\n"
          " [-t|--target path]       file system location to save received "
          "files to\n"
          " [-u|--udp ip:port]       a udp ip to send to (first is self to "
          "bind to)\n"
          " [-w|--max-wait time]     maximum time to wait in seconds (double "
          "format)\n"
          " [-z|--target-id id]      id of the entity that must acknowledge "
          "receipt\n"
...
void write_file(knowledge::KnowledgeMap& records,
    const madara::transport::TransportContext&, knowledge::Variables& vars)
{
...
int main(int argc, char** argv)
{
  ...
  knowledge.set(".target", target_id, knowledge::EvalSettings::SEND);

  ack = knowledge.get_ref(knowledge.expand_statement("file.{.id}.ack"));

  if(is_terminator)
    knowledge.set("terminated", 1.0, knowledge::EvalSettings::SEND);
  ...
  if(settings.id == 0)
  {
    ...
    knowledge.print("Writing {file_name} to network. "
    ...
    knowledge.evaluate("terminated = 1", knowledge::EvalSettings::SEND);
