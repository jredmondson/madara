...
void test_system_calls(madara::knowledge::KnowledgeBase& knowledge)
{
  ...
  knowledge.evaluate("two_to_4th = #pow(2, 4);");

  knowledge.evaluate("tan_1_1 = #tan(1.1);");

  knowledge.evaluate("begin_s = #get_time_seconds();"
                     "begin_ns = #get_time();"
                     "#sleep(3);"
  ...
  KnowledgeRecord::Integer diff_s = knowledge.get("diff_s").to_integer();
  ...
  knowledge.evaluate("#set_scientific()");

  knowledge.print();

  std::cerr << "Output being reverted with #set_fixed...\n";

  knowledge.evaluate("#set_fixed()");
