  ...
  for(int i = 1; i < argc; ++i)
  {
    ...
    else
    {
      madara_logger_ptr_log(logger::global_logger.get(), logger::LOG_ALWAYS,
          ...
          " [-c|--consumers consumers] the number of information consumerss to "
          "start\n"
          " [-f|--logfile file]      log to a file\n"
          " [-l|--level level]       the logger level (0+, higher is higher "
          "detail)\n"
          " [-p|--producers producers] the number of information producers to "
          "start\n"
          " [-c|--consumers consumers]   the number of reader threads to "
          "start\n"
...
int main(int argc, char** argv)
{
  ...
  std::cerr << "The consumers completed " << *jobs_completed << " jobs\n";
