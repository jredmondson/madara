...
void test_expansion (madara::knowledge::KnowledgeBase & knowledge)
{
  ...
  assert (knowledge.get ("v22") == (madara::KnowledgeRecord::Integer)47);

  result = knowledge.evaluate (".var{.var2}{.var2}");
  assert (result == (madara::KnowledgeRecord::Integer)2);

  result = knowledge.evaluate (".var{.var2}is{.var2}");
  assert (result == (madara::KnowledgeRecord::Integer)1);

  // test variables in conditionals
  knowledge.evaluate ("var{.var3} = .var{.var2}");
  assert (knowledge.get ("var3") == (madara::KnowledgeRecord::Integer)2);
